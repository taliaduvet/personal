"use client";

import { useState } from "react";
import type { WorkMode } from "@/lib/types";
import { uniqueId } from "@/lib/slug-id";

type Props = {
  workModes: WorkMode[];
  onSave: (mode: WorkMode) => void;
  onRemove: (id: string) => string | null;
};

export function WorkModesSettings({ workModes, onSave, onRemove }: Props) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [name, setName] = useState("");
  const [error, setError] = useState<string | null>(null);

  function startNew() {
    setEditingId("__new__");
    setName("");
    setError(null);
  }

  function startEdit(mode: WorkMode) {
    setEditingId(mode.id);
    setName(mode.name);
    setError(null);
  }

  function cancel() {
    setEditingId(null);
    setName("");
    setError(null);
  }

  function save() {
    const trimmed = name.trim();
    if (!trimmed) {
      setError("Enter a name.");
      return;
    }
    const id =
      editingId === "__new__"
        ? uniqueId("mode", trimmed, new Set(workModes.map((m) => m.id)))
        : editingId!;
    onSave({ id, name: trimmed });
    cancel();
  }

  function remove(id: string) {
    const err = onRemove(id);
    setError(err);
    if (!err && editingId === id) cancel();
  }

  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-display text-base font-semibold text-ink">Work modes</h2>
          <p className="mt-1 text-sm text-muted">
            How you stamp days and classify tasks — Creative, Admin, Outreach, and your own. Not
            every life area needs one — see Area override in day planning instead.
          </p>
        </div>
        {!editingId && (
          <button
            type="button"
            onClick={startNew}
            className="shrink-0 rounded-md border border-border bg-canvas px-2.5 py-1 text-xs font-medium text-accent hover:border-accent/40"
          >
            + Add mode
          </button>
        )}
      </div>

      <ul className="mt-4 space-y-2">
        {workModes.map((mode) => (
          <li
            key={mode.id}
            className="flex items-center gap-3 rounded-lg border border-line bg-canvas/40 px-3 py-2.5"
          >
            <span className="min-w-0 flex-1 text-sm font-medium text-ink">{mode.name}</span>
            <button
              type="button"
              onClick={() => startEdit(mode)}
              className="text-xs text-muted hover:text-ink"
            >
              Edit
            </button>
            {workModes.length > 1 && (
              <button
                type="button"
                onClick={() => remove(mode.id)}
                className="text-xs text-muted hover:text-danger"
              >
                Remove
              </button>
            )}
          </li>
        ))}
      </ul>

      {editingId && (
        <div className="mt-4 rounded-lg border border-accent/30 bg-accent-soft/20 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-faint">
            {editingId === "__new__" ? "New work mode" : "Edit work mode"}
          </p>
          <label className="mt-2 block">
            <span className="text-[10px] font-medium uppercase tracking-wide text-faint">Name</span>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="mt-1 w-full rounded-md border border-border bg-canvas px-2.5 py-1.5 text-sm text-ink outline-none focus:border-accent"
            />
          </label>
          <div className="mt-3 flex gap-2">
            <button
              type="button"
              onClick={save}
              className="rounded-md bg-accent px-3 py-1.5 text-xs font-medium text-white hover:opacity-90"
            >
              Save
            </button>
            <button type="button" onClick={cancel} className="text-xs text-muted hover:text-ink">
              Cancel
            </button>
          </div>
        </div>
      )}

      {error && <p className="mt-2 text-xs text-[#bc6740]">{error}</p>}
    </div>
  );
}
