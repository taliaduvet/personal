...
export type DayShapePanelProps = {
  ...
  /** Flat list is open-day vs mode-day; hide mode/area pills to match desk. */
  isOpenDay?: boolean;
};
...
export function DayShapePanel({
  ...
  const placeButton = (taskId: string) => (
    <button
      type="button"
      onClick={() => setSelectedTaskId((id) => (id === taskId ? null : taskId))}
      className={[
    ...
    >
      {selectedTaskId === taskId ? "Placing…" : "Place"}
    </button>
  );
    ...
    <div className="space-y-4">
      <header className="space-y-2">
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            <h2 className="font-display text-sm font-semibold text-ink">Your day</h2>
            {commitment.blocked ? (
              <p className="mt-0.5 text-xs font-medium text-[#bc6740]">Marked as blocked</p>
            ) : commitment.timedHours > 0 ? (
              <p className="mt-0.5 text-xs text-muted">
                <span className="font-medium text-ink">{commitment.timedHours}h</span> on calendar
              </p>
            ) : (
              <p className="mt-0.5 text-xs text-muted">Light calendar day · morning · afternoon · evening</p>
            )}
          </div>
        </div>
        {commitment.timedHours > 0 && (
      ...
      <label className="block">
        ...
        <textarea
          ...
          className="mt-1.5 w-full resize-none rounded-lg border border-border bg-canvas/40 px-2.5 py-2 text-sm text-ink outline-none focus:border-accent"
          maxLength={400}
        />
      </label>
      ...
      {unassigned.length > 0 && (
        <section>
          <p className="text-[10px] font-semibold uppercase tracking-wide text-faint">
            Unplaced · Place, then tap a column
          </p>
          <div className="mt-2 space-y-2">
            {unassigned.map((task) => (
              <div key={task.id} className="space-y-1">
                ...
                />
                <div className="flex justify-end px-1">{placeButton(task.id)}</div>
              </div>
            ))}
          </div>
        </section>
      ...
      <section>
        <p className="text-[10px] font-semibold uppercase tracking-wide text-faint">Your day</p>
                      ...
                      return (
                        ...
                        >
                          ...
                          <select
                            ...
                            className="mt-1 w-full rounded border border-border bg-canvas px-1.5 py-0.5 text-[10px] text-ink"
                          >
                ...
                {blockEvents.length === 0 &&
                  ...
                  !(block === "morning" && commitment.allDayEvents.length > 0) && (
                    <p className="mt-2 text-xs text-faint">
                      {isDropTarget ? "Tap to place here" : "—"}
                    </p>
                  )}
              </div>
            );
      ...
      {selectedTaskId && (
        <p className="text-xs text-accent">
          Tap a time block to place this task · tap Place again to cancel
        </p>
      )}

      {benchTasks.length === 0 && (
        <p className="py-4 text-center text-sm text-muted">Nothing on the bench to shape yet.</p>
      )}
    </div>
  );
}
