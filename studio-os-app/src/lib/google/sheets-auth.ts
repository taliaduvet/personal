...
const STORAGE_TOKEN_KEY = "studio-os.gsheet-token.v1";
...
export function isSheetsDirectConnected(): boolean {
  return Boolean(getSheetsAccessToken());
}

export function disconnectSheetsDirect() {
  cachedToken = null;
  tokenExpiry = 0;
  localStorage.removeItem(STORAGE_TOKEN_KEY);
}
...
export function getStoredSheetsClientId(): string {
  return getStoredCalendarClientId() || GOOGLE_CLIENT_ID;
}

export function saveSheetsClientId(id: string) {
  saveCalendarClientId(id);
}

/** Connect Google Sheets via GIS (read + write). */
export async function connectSheetsDirect(clientId?: string): Promise<string> {
  const id = (clientId ?? getStoredSheetsClientId()).trim();
  if (!id) {
    throw new Error("Add a Google Client ID in Settings first.");
  }
  saveSheetsClientId(id);
  return requestToken(id, "consent");
}

export async function refreshSheetsDirectSilent(): Promise<string | null> {
  const id = getStoredSheetsClientId();
  if (!id) return null;
  try {
