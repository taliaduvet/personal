...
*Last updated: September 22, 2026 (per-day task slotting + Today "start your day" gate + Dashboard/Today sync fix; a stale "Get set up" card fix; a Google OAuth refresh-token fix; and an app-wide functionality audit that caught a cloud-delete race and a stale recipe deadline — see App shell below)*  
...
- **Completed tasks reverting to "todo"** — `CloudSyncBridge.tsx`'s `mergeCloud` let cloud unconditionally win over local on every non-seed pull (fired on every tab focus/visibility change, not just app reload). The cloud push queue debounces 800ms before writing to Supabase, so a pull that landed before that write completed (tab switch right after checking a task off, laptop closing before the debounce fired, etc.) would pull back the still-`todo` cloud row and stomp the local completion — reading as "I marked it done and it came back." The fix already existed for settings (`hasPendingCloudOp("sos_settings", "singleton")` guards the settings merge) but was never applied to tasks/projects/recipes. Fixed Aug 31, 2026: the same `hasPendingCloudOp()` guard now covers the tasks, projects, and recipes merges — a pull no longer overwrites a row that still has a push in flight.
- **Mode-day overwhelm (no per-day capacity signal) + Dashboard/Today drift.** Every day stamped with a work mode showed the *entire* approved+matching pool for that mode identically — 11 admin tasks looked the same on day 1 of a 3-admin-day stretch as on day 3, with no way to say "these 4 are for Monday." Separately, Dashboard's "Today's focus" card computed its list from the raw `inToday` boolean while Today itself computed a richer mode-bench set that mostly ignored that flag, so the two screens could disagree on a mode day. Fixed Sep 21, 2026, in three parts:
  - **Per-day task slotting** — a new optional step in the week-planning wizard (`WeekPlaceTasksPanel.tsx`, step 3) lets you place specific approved tasks onto specific days within a mode's stamped days, mirroring the existing Shape Day bench/place/unplace gesture (`day-shape.ts` → new `day-slots.ts`, keyed by `dateKey` instead of `DayShapeBlock`). Two new fields on `WeekDayFocusEntry` (`src/lib/week-focus.ts`): `slottedTaskIds` and `planConfirmedAt`. `tasksForTodayModeBench()` takes an optional `slotting` param (via new `modeSlottingForToday()`) — once any day in a mode has an explicit placement this week, every day of that mode switches from "whole pool" to "only what's slotted here" (in-progress/carried tasks always bypass the filter); weeks that skip this step behave exactly as before.
  - **Today's "start your day" gate** (`src/components/today/DayStartGate.tsx`) — on a mode day, before the normal workspace, Today shows just what's slotted for that day with a one-tap "Looks right →" confirm (writes `planConfirmedAt`) and a "swap one in" picker (reuses the Unplanned Nudge approve/dismiss pattern) for when reality doesn't cooperate. Confirmation is per-calendar-day, persists through the same settings-store path as everything else in `WeekDayFocusEntry`, and the gate is skipped once confirmed or on open days.
  - **Shared bench computation** — new `computeTodayBench()` in `week-focus.ts` is now the one source of truth both `TodayView.tsx` and `DashboardView.tsx` call, so Dashboard's "Today's focus" preview and "In Today" count can no longer drift from what Today actually shows.
- **"Get set up" card never went away even when signed in.** `OnboardingCard.tsx`/`SetupBanner.tsx` only checked Google-*Sheet*-connection state (`connection`/`cloudPrimary` from `useSheet()`), not whether the user was actually signed in — an already-built `useGoogleSignedIn()` hook (`src/lib/google/use-google-access-token.ts`, reads the real Supabase auth session) existed but nothing on the Dashboard used it. Fixed Sep 21, 2026: both components now also hide once `useGoogleSignedIn()` is true.
- **Google kept asking for re-login during normal use.** Root cause: the shared PKCE OAuth helper (`src/lib/google/gis-oauth.ts`) used by the Calendar/Contacts/Drive/Unified "direct connect" flows never requested `access_type=offline`, so Google never issued a refresh token — every access token was ~1hr, and the only fallback was Google's flaky browser-session-based silent reauth. A complete, correct refresh mechanism already existed (`/api/google-token-refresh`, an httpOnly-cookie-based refresh endpoint) but was 100% dead code — the cookie it depended on was never set. Fixed Sep 21, 2026: `buildGoogleOAuthUrl()` now requests `access_type=offline`; `/api/google-oauth-callback` captures the resulting `refresh_token` into the (previously unused) `OAUTH_REFRESH_COOKIE`; a new `refreshViaCookie()` helper (`oauth-refresh-client.ts`) is now tried first by every silent-refresh function (`calendar-auth.ts`, `contacts-auth.ts`, `drive-auth.ts`, and a brand-new `refreshGoogleUnifiedSilent()` in `google-unified-auth.ts`, which had *no* refresh path at all before this), falling back to the old GIS-session reauth only if the cookie refresh fails. Since a refresh token is scope-bound and there's one shared cookie, Calendar/Contacts/Drive's own connect flows now request the full `GOOGLE_UNIFIED_SCOPES` bundle (not just their own narrow scope) so the one refresh token actually covers whichever service asks — this also means Google's consent screen now lists all four scopes on any one of those buttons, and Contacts' own connect gains real write access (previously read-only; needed for the already-existing `createGoogleContact()`). All four `disconnect*()` functions now also clear the shared cookie. **⚠️ Known external dependency, not fixable in code:** if this app's Google Cloud OAuth consent screen is still in "Testing" publishing status, Google unconditionally expires refresh tokens after 7 days regardless of this fix — worth checking Google Cloud Console if the problem resurfaces after about a week. Also explicitly deferred: `sheets-auth.ts` uses a structurally different, refresh-incapable GIS API and needs its own rewrite to benefit from any of this.
- **App-wide functionality audit (Sep 22, 2026).** Five parallel passes across every feature area (Today/Dashboard/week planning, Tasks/Projects/Archive/Inbox, Journal/Habits/Practice/Weekly Review, Settings/Google/Sheet sync/Auth, Trust Core/deadlines/nudges) looking for real behavioral bugs, not style nitpicks. Two confirmed and fixed:
  - **Deleting a task/project/recipe while cloud-synced could silently un-delete itself.** `CloudSyncBridge.tsx`'s `mergeCloud` handled the "in-flight push" race for *updates* (see the Aug 31 fix above) but not for *deletes*: a delete removes the row locally and queues a tombstone write (debounced 800ms, same as any other cloud write), but if a pull landed before that tombstone reached Supabase, the merge saw "cloud still has it, local doesn't" and restored it — the delete would appear to silently fail on the next tab focus. Fixed: the same `hasPendingCloudOp()` guard now also covers the cloud-only branch for tasks, projects, and recipes, so a pull can no longer resurrect a row whose delete is still in flight.
  - **Reapplying a recipe could silently keep a task's old deadline.** `applyRecipeMilestones()` (`src/lib/recipes.ts`) recomputed `deadlineInDays` correctly on every reapply, but if that task's deadline had ever been manually edited (which sets an absolute `deadlineDateKey`), the stale key survived the spread untouched — and several readers (`nudges.ts`, `trust/commitments.ts`, `trust/completeness.ts`, the briefing route) prefer `deadlineDateKey` over `deadlineInDays` when both are present, so they'd keep showing/acting on the old manually-set date instead of the recipe's new one. Fixed: `deadlineDateKey` is now explicitly cleared when a linked task's deadline is recomputed from the recipe, matching how a newly-created milestone task already behaves.
  - Two other reported findings were investigated and found to be correct existing behavior, not bugs: `settings-store.tsx`'s Sheet `_AppData` pull deliberately treats an empty incoming `contacts`/`lifeAreas`/`weekPlanning` as "no signal, keep local" rather than "user cleared everything" (consistent across all three fields, and the safer read of an ambiguous empty pull); and `JournalView.tsx`'s cross-year month navigation (`setMonth` + `setYear` in the same handler) is safe under React 19's automatic batching.
  - Everything else audited — Today/Dashboard/week planning, the Trust Core completeness invariant, waiting-on/needs-respond, session nudges, push notifications — came back clean.
...
- **Session timer + transition warning + break-habit loop** — core is committed and live. **Push/Supabase send-side portion not yet started** (see below).
  ...
  - **Reliability fix**: a user ran a full 90-minute timed session with the Studio OS tab visible on a second monitor (not the focused window) the whole time and got zero warning/times-up nudges. Root cause wasn't app logic — `timedNudgeDue`/`msUntilNextTimedEvent` are correct and tested — it's that the whole mechanism rested on a single precise `setTimeout` that browsers can throttle or drop in an unfocused/backgrounded window over that long a delay, with no fallback. Fixed in `sessions-store.tsx`: nudge-firing was extracted into a stable `fireDueNudge()` (reads `activeSession`/`tasks` through refs so it's safe to call from multiple places), now invoked from three paths — the original precise `setTimeout` (snappy when the tab is fully active), the existing 60s display-tick `setInterval` (catches a due nudge within ~60s even if the precise timer got throttled), and a new `visibilitychange` listener (catches up immediately when the tab regains focus/visibility). Also added `src/lib/browser-notify.ts` — fires a real OS `Notification` alongside the existing in-app banner + beep whenever a nudge is due, so it's visible regardless of which window has focus; permission is requested once, at the moment a timed session actually starts (reuses the same `Notification.permission` the Dashboard's existing "Turn on notifications" button grants — this is a local notification, distinct from the unbuilt server-push path above, and needs no service worker or VAPID key). Nudge copy was deduplicated into `session-nudge.ts`'s `nudgeMessage()` so the in-app banner and the OS notification can't drift apart. Verified live: simulated a stale `ActiveSession` past both the warning and times-up thresholds via localStorage and confirmed each banner + state latch fires correctly on load.
...
- **User-editable work modes** — `WORK_MODES` (Admin/Creative/Outreach/Errands) was a hardcoded array imported directly from `sample-data.ts` everywhere (`TaskClassify.tsx`, `DayPlanningPanel.tsx`, `today/DayShapePanel.tsx`, `WeekPlanningOverlay.tsx`, `parse.ts`, `lenses.ts`) — no registry, no settings UI, unlike life areas and projects. Added `src/lib/work-mode-registry.ts` (mirrors `life-area-registry.ts`), a `workModes: WorkMode[]` field on `AppSettings` with `upsertWorkMode`/`removeWorkMode` (keep-at-least-one invariant, same shape as life areas), a new `WorkModesSettings.tsx` CRUD panel wired into `SettingsView.tsx`, and cloud-merge protection in `settings-merge.ts` (`isSeedWorkModes`/`mergeWorkModes`, same seed-vs-edited logic as life areas) plus `CloudSyncBridge.tsx`. `parse.ts`'s `admin:`-style prefix shorthand is now built from the live mode list instead of a hardcoded regex. Verified live: added a mode in Settings, saw it immediately in the capture Mode dropdown, removed it, confirmed persistence in `localStorage`.
- **Task capture no longer discards manual field picks on Confirm** — `TaskDetailSheet.tsx`'s `confirmCapture()` re-ran `parseTaskTitle()` on the raw text and unconditionally patched `projectId`/`lifeAreaId`/`workModeId`/`doPlan`/`deadlineInDays` from the fresh parse, silently overwriting anything the user had just picked via the classify dropdowns (which had already been saved instantly through `setClassify` → `updateTask`). The live-update effect a few lines above already had the correct pattern — check `manualClassify.current` before applying a parsed field — but Confirm skipped it entirely. Fixed by routing Confirm through the existing `taskWithCaptureParse()` helper (already used for the live preview) so manually-set fields are preserved and only untouched fields take the fresh parse. Verified live: captured "Email venues tomorrow" (parses to mode Outreach), manually overrode mode to Admin, confirmed the saved task kept `workModeId: "admin"`. Audited every other task-creation entry point (blank/New task, Inbox capture, `/api/capture`, sheet import/sync, recipe-generated tasks) — no other field-loss bugs found; `updateTask` always does a full `{...current, ...patch}` merge so partial patches can't wipe unrelated fields.
- **Life areas assignable to days, not just work modes** — day focus (`DayFocus` in `week-focus.ts`) only supported a work-mode set or a project override; an area like Home with no natural work mode had no way to claim a day. Added a `{kind: "area", id: string}` variant (mirroring the existing `DayShapeIntent` union that already anticipated this but was never wired into `DayFocus`), extended `focusLabel`/`taskMatchesFocus`/`focusEquals`/`dayFocusIncludes` to handle it, and added an "Area override" picker next to the existing project override in `DayPlanningPanel.tsx` and `today/DayShapePanel.tsx`, plus a life-area brush (mutually exclusive with the mode brush) in `WeekPlanningOverlay.tsx`'s Place-modes step. Verified live: stamped Today with the Home & Admin area via the day panel, confirmed the header and bench correctly show/filter by area; stamped Tomorrow with the same area from the week-planning wizard's multi-day brush.
- **Projects can be marked complete** — `Project` (`types.ts`) had no status concept at all; the only way to get one out of the active list was `deleteProject`, which permanently wipes it (and only works for locally-created projects, not sheet-imported ones). Added `status?: "active" | "done"` and `completedAt?: string | null` to `Project`, and `completeProject`/`reopenProject` on `projects-store.tsx` (writes through the existing per-project override mechanism, so it works for seed/sheet/local projects alike and survives sheet re-imports the same way name/why overrides already do). `ProjectDetailView.tsx` gets a "Mark complete"/"Reopen project" button plus a "Completed" badge and date. `ProjectsView.tsx` moves completed projects out of each life area's main list into a collapsed "Completed (N)" `<details>` section (mirroring the existing done-tasks pattern), with a link to the new **Archive → Projects** tab (`CompletedProjectsView.tsx`, wired into `ArchiveView.tsx` and the `?tab=projects` query param) that lists every completed project across all areas with a life-area filter. Completed projects are filtered out of the three "assign new work" pickers (`TaskClassify.tsx`'s project dropdown, `DayPlanningPanel.tsx` and `today/DayShapePanel.tsx`'s project-override chips) but a task/day already pointing at one still shows it, so nothing looks broken after the fact — the project detail page, task lookups (`activeProjectName`/`activeProjectWhy` via the unfiltered project registry), and Shelf/session history are all unaffected and keep resolving completed projects normally. Verified live: marked Fall Tour complete, confirmed it disappeared from the main Projects list and the task-capture project picker, confirmed it appeared in the Projects page's collapsed Completed section and in Archive → Projects with the right life area and date, then reopened it and confirmed it returned to normal.
...
guard in `src/proxy.ts`. **`src/lib/` — the whole trust core, 47 test files, 339 tests —
...
| 1.3 | Move `src/lib/` across intact | The trust core + 339 tests. Should be near-lift-and-shift; treat any test that *needs* changing as a signal something was framework-coupled that shouldn't have been. |
...
| Onboarding | A stranger has none of Talia's mental model. Empty states, first-run guidance, and an explanation of the mode bench / do-plan / dormant-vs-triggered vocabulary the whole app assumes. **Also needs a "Import your Marrow Sheet" path (flagged 2026-08-31, see "Missing / partial features" above) — without it, upgrading from the free Sheet means starting over, which undercuts the whole free→paid funnel.** |
...
| Import from Marrow (free Sheet template) | ❌ Not built | **Flagged 2026-08-31 — revisit before public launch.** Marrow (the free Sheets giveaway, formerly "Studio OS") and this app's Sheet-sync have diverged: `schema.ts`/`map.ts` still expect the old template's columns (`Category`/`Priority`/`Goal`/`Target Week`, Projects' `Detail`/`Color`) and onboarding points at a separate `TEMPLATE_SHEET_ID`, not a user's actual Marrow copy. Header validation fails on a real Marrow sheet — someone who's used Marrow for months would have to re-enter everything to upgrade. Don't force the app's richer schema down to Marrow's simpler one; instead build a one-time "Import your Marrow Sheet" onboarding step that reads Tasks/Projects once, maps what exists, and defaults the app-only fields. This is a conversion-funnel risk, not just a tech gap — see Phase 3 "Onboarding" below. |
