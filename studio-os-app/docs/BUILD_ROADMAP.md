...
*Last updated: September 21, 2026 (per-day task slotting + Today "start your day" gate + Dashboard/Today sync fix; a stale "Get set up" card fix; and a Google OAuth refresh-token fix that stops repeated Google re-login prompts — see App shell below)*  
...
guard in `src/proxy.ts`. **`src/lib/` — the whole trust core, 47 test files, 337 tests —
...
| 1.3 | Move `src/lib/` across intact | The trust core + 337 tests. Should be near-lift-and-shift; treat any test that *needs* changing as a signal something was framework-coupled that shouldn't have been. |
