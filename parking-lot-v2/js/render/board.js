...
import { getPiles, INBOX_PILE_ID } from '../domain/piles-people.js';
 ...
 * @param {(id: string) => void} d.openTaskHistory
  ...
  function renderColumns() {
    ...
    if (isPilesView) {
      ...
      container.innerHTML = pileColumns.map((col, colIdx) => {
        ...
        const items = withoutToday(getActiveItems().filter(i =>
        ...
        const pileIdAttr = ` data-pile-id="${col.pileId}"`;
    ...
    container.querySelectorAll('.btn-task-history').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        d.openTaskHistory(btn.dataset.id);
      });
    });
