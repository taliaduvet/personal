...
import { getPileName, getPersonName, INBOX_PILE_ID, ensureInboxPile } from './piles-people.js';
...
function createItem(text, category, deadline, priority, recurrence, reminderAt, doingDate, pileId, friction, personId) {
  ...
  // Fall back to inbox so nothing ends up with no pile
  if (!resolvedPileId) {
    ensureInboxPile();
    resolvedPileId = INBOX_PILE_ID;
  }
  if (!resolvedPersonId) {
