...
import { getPileName, getPiles, INBOX_PILE_ID } from '../domain/piles-people.js';
import {
  isConnected as calIsConnected,
  getWeekEvents,
  calcFreeHours
} from './calendar.js';
import {
...
export function createWeekPlanningUI(d) {
  ...
  function renderPilesReference() {
    ...
    const cols = pileColumns
      .map((col) => {
        const isInbox = col.pileId === INBOX_PILE_ID;
        const items = withoutToday(active.filter((i) =>
          isInbox ? (i.pileId === INBOX_PILE_ID || i.pileId == null) : i.pileId === col.pileId
        ));
        const sorted = sortByTimeBandsAndFriction(items);
        const pileIdAttr = ` data-pile-id="${escapeHtml(col.pileId)}"`;
        ...
        const addBtnAttr = ` data-pile-id="${escapeHtml(col.pileId)}"`;
  ...
  // ── Google Calendar week capacity strip ───────────────────────────────────

  async function renderCalendarWeekStrip(weekKeys) {
    const strip = document.getElementById('gcal-week-strip');
    if (!strip) return;
    if (!calIsConnected()) { strip.style.display = 'none'; return; }

    strip.style.display = 'flex';
    try {
      const monday = new Date(weekKeys[0] + 'T00:00:00');
      const events = await getWeekEvents(monday);
      if (!events) { strip.style.display = 'none'; return; }

      strip.innerHTML = weekKeys.slice(0, 7).map(dateKey => {
        const freeH = calcFreeHours(events, dateKey);
        const pct = Math.round((freeH / 8) * 100);
        const barClass = freeH >= 5 ? 'gcal-day-cap-bar--high' : freeH >= 2.5 ? 'gcal-day-cap-bar--mid' : 'gcal-day-cap-bar--low';
        const dayName = new Date(dateKey + 'T12:00:00').toLocaleDateString([], { weekday: 'short' });
        return `<div class="gcal-day-cap" title="${dayName}: ~${freeH}h free">
          <span class="gcal-day-cap-label">${dayName}</span>
          <div class="gcal-day-cap-bar-wrap"><div class="gcal-day-cap-bar ${barClass}" style="width:${pct}%"></div></div>
          <span class="gcal-day-cap-free">${freeH}h</span>
        </div>`;
      }).join('');
    } catch (e) {
      console.warn('[calendar] week strip error', e);
