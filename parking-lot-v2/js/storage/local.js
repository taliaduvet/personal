...
export function loadState() {
  try {
    ...
    if (stored) {
      ...
      if (typeof parsed.weekTheme === 'string' || parsed.weekTheme === null) {
        state.weekTheme = parsed.weekTheme ?? null;
      }
      if (typeof parsed.planningStreak === 'number') state.planningStreak = parsed.planningStreak;
      if (typeof parsed.lastPlanningWeekMonday === 'string') state.lastPlanningWeekMonday = parsed.lastPlanningWeekMonday;
      if (Array.isArray(parsed.weekWins)) state.weekWins = parsed.weekWins.slice(0, 3);
  ...
  try {
    localStorage.setItem(STORAGE_PREFIX + 'data', JSON.stringify({
      ...
      hiddenFromTodayByDate: state.hiddenFromTodayByDate && typeof state.hiddenFromTodayByDate === 'object'
        ...
        : {},
      notes: state.notes || [],
      weekTheme: state.weekTheme || null,
      planningStreak: state.planningStreak || 0,
      lastPlanningWeekMonday: state.lastPlanningWeekMonday || null,
      weekWins: state.weekWins || [],
