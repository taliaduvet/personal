...
function wireComposer() {
  ...
  function bindEvents() {
    ...
    if (addBtn) addBtn.addEventListener('click', () => modalApi.openAddModal());
        ...
        if (shortcutsOverlay && shortcutsOverlay.style.display === 'flex') {
          ...
          for (const id of modals) {
            ...
            if (m && m.style.display === 'flex') {
              if (id === 'add-modal') modalApi.closeAddModal();
              else if (id === 'edit-modal') modalApi.closeEditModal();
    ...
    if (closeAdd) closeAdd.addEventListener('click', () => modalApi.closeAddModal());

    const addModal = document.getElementById('add-modal');
    if (addModal) addModal.addEventListener('click', (e) => {
      if (e.target.id === 'add-modal') modalApi.closeAddModal();
    });

    const taskInput = document.getElementById('task-input');
    if (taskInput) {
      taskInput.addEventListener('input', () => modalApi.applySmartFields());
      taskInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') modalApi.addSingle();
      });
    }

    const submitSingle = document.getElementById('submit-single');
    if (submitSingle) submitSingle.addEventListener('click', () => modalApi.addSingle());
    const submitQuick = document.getElementById('submit-quick');
    if (submitQuick) submitQuick.addEventListener('click', () => modalApi.addQuick());
    ...
    modalApi.initVoiceMulti();

    const closeEdit = document.getElementById('close-edit');
    if (closeEdit) closeEdit.addEventListener('click', () => modalApi.closeEditModal());

    const editModal = document.getElementById('edit-modal');
    if (editModal) editModal.addEventListener('click', (e) => {
      if (e.target.id === 'edit-modal') modalApi.closeEditModal();
    });

    const saveEditBtn = document.getElementById('save-edit');
    if (saveEditBtn) saveEditBtn.addEventListener('click', () => modalApi.saveEdit());
    ...
    if (editTextEl) editTextEl.addEventListener('input', () => modalApi.applySmartFieldsToEdit());
    ...
    function renderRelationshipsDetail(personId) {
      ...
      content.querySelectorAll('.relationships-open-task').forEach(function(btn) {
        btn.addEventListener('click', function() {
          closeRelationshipsPanel();
          modalApi.openEditModal(btn.dataset.id);
        });
      });
