...
import {
  ...
  getClientId as calGetClientId,
  connect as calConnect,
  disconnect as calDisconnect,
  getTodayEvents,
  formatEventTime,
  formatEventRange,
  calcFreeHours
...
function wireComposer() {
  ...
  function ensureSettingsAccordion() {
    ...
    const gcalSection = document.getElementById('settings-gcal');
    ...
    if (gcalSection) createSection('Calendar', false, [gcalSection]);
  ...
  async function renderCalendarTodayStrip() {
    const strip = document.getElementById('gcal-today-strip');
    if (!strip) return;
    if (!calIsConnected()) { strip.style.display = 'none'; return; }

    strip.style.display = 'flex';
    strip.innerHTML = '<span class="gcal-loading">Loading…</span>';
  ...
  function bindCalendarSettings() {
    const guideLink = document.getElementById('gcal-setup-guide-link');
    const guide = document.getElementById('gcal-setup-guide');
    const originEl = document.getElementById('gcal-guide-origin');
    ...
    if (clientInput) clientInput.value = calGetClientId();

    function updateCalStatus() {
      if (!statusEl || !disconnectBtn || !connectBtn) return;
      if (calIsConnected()) {
        statusEl.textContent = '✓ Connected to Google Calendar';
        statusEl.style.color = 'var(--positive, #3a9a6e)';
        disconnectBtn.style.display = 'inline-block';
        connectBtn.textContent = 'Reconnect';
      } else if (calGetClientId()) {
        statusEl.textContent = 'Token expired — click Connect to re-authorize.';
    ...
    if (guideLink && guide) {
      guideLink.addEventListener('click', e => {
        ...
        guideLink.textContent = guide.style.display === 'none' ? 'show me how →' : 'hide guide ↑';
      });
    }

    connectBtn.addEventListener('click', async () => {
      const id = (clientInput?.value || '').trim();
      if (!id) { if (statusEl) { statusEl.textContent = 'Paste your Client ID first.'; statusEl.style.color = 'var(--negative)'; } return; }
      connectBtn.disabled = true;
      if (statusEl) { statusEl.textContent = 'Opening Google authorization…'; statusEl.style.color = 'var(--text-muted)'; }
      try {
        await calConnect(id);
        updateCalStatus();
