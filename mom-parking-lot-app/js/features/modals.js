...
import { parseRecurrenceRuleInput, formatRecurrenceRuleInput } from '../domain/recurrence.js';
...
export function createModalController(deps) {
  ...
  function openEditModal(id) {
    ...
    if (IS_MOM_APP && editNotes && !editNotes.dataset.noteToTaskWired) {
      ...
      editNotes.dataset.noteToTaskWired = '1';
    }
  ...
  function saveEdit() {
    ...
    const editNotes = document.getElementById('edit-notes');
    if (editNotes) item.notes = editNotes.value.trim() || null;
