...
import { wireNoteToTask } from '../features/note-to-task.js';
  ...
  function renderColumns() {
    ...
    if (isPilesView) {
          ...
          <div class="column column-accent" data-category="${col.id}"${pileIdAttr} style="--column-accent: ${accent}">
            ...
            <div class="column-items">
              ${sorted.length ? sorted.map(item => renderTaskCard(item, { showLifeAreaAsTag: true })).join('') : `
                <div class="empty-state column-add-hint" data-category="${col.id}"${pileIdAttr}>No tasks in this project</div>
              `}
            </div>
      ...
      container.innerHTML = cats.map(catId => {
        ...
        const rawNote = IS_MOM_APP ? getColumnNoteHtml(catId) : (d.state.columnNotes && d.state.columnNotes[catId]) || '';
        const noteContent = IS_MOM_APP ? String(rawNote).replace(/<[^>]+>/g, '') : rawNote;
      ...
      container.querySelectorAll('.column-note-textarea').forEach(ta => {
        ...
        const wrap = ta.closest('.column-note-textarea-wrap') || ta.parentElement;
        if (IS_MOM_APP && wrap) {
          wireNoteToTask({
            root: wrap,
            editor: ta,
            ...
            onTaskCreated: () => renderColumns()
          });
        }
        ta.addEventListener('input', () => {
          const cid = ta.dataset.category;
          if (d.state.columnNoteSaveTimeouts[cid]) clearTimeout(d.state.columnNoteSaveTimeouts[cid]);
          d.state.columnNoteSaveTimeouts[cid] = setTimeout(() => {
            if (IS_MOM_APP) {
              setColumnNoteHtml(cid, ta.value);
            } else {
              if (!d.state.columnNotes) d.state.columnNotes = {};
              d.state.columnNotes[cid] = ta.value;
            }
            delete d.state.columnNoteSaveTimeouts[cid];
            d.saveState();
