...
export function renderTaskCard(item, opts) {
  ...
  return `
      <div class="task-card ${overdue ? 'overdue' : ''} ${checked ? 'selected' : ''} ${daysParked >= 30 ? 'stale-nudge' : ''}" data-id="${item.id}"${staleNudge}>
        <span class="task-drag-handle" draggable="true" data-id="${item.id}" title="Drag to move between columns or to Today · Click to select for Today" aria-label="Drag or select task">⋮⋮</span>
