# Deploy Mom's Parking Lot to GitHub Pages

Deployed with the Personal monorepo workflow at **`/personal/mom-parking-lot-app/`**.

Optional GitHub Secrets (fall back to couple app secrets if unset):

| Secret | Purpose |
|--------|---------|
| `MOM_SUPABASE_URL` | Mom app Supabase project URL |
| `MOM_SUPABASE_KEY` | Mom app anon key |

Run [supabase-mom-setup.sql](./supabase-mom-setup.sql) on the mom Supabase project for device sync.

---

# Deploy Couples Parking Lot to GitHub Pages (reference)
